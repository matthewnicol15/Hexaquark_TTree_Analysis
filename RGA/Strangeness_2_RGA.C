...
void Strangeness_2_RGA(){
  ...
  TFile *f = new TFile("/mnt/f/PhD/Trees/Dibaryon/RGA/Strangeness_2/RGA_FALL2018_Inbending_skim4_S2_Tree_290921_01.root");
  // TFile *f = new TFile("/media/mn688/Elements1/PhD/Trees/Dibaryon/RGA/Strangeness_2/RGA_SPRING2019_Inbending_skim4_S2_Tree_050721_01.root");
  // TFile *f = new TFile("/mnt/f/PhD/Trees/Dibaryon/RGA/Strangeness_2/RGA_SPRING2019_Inbending_skim4_S2_Tree_050721_01.root");
  // Read TTree within root file and assign it to 't1'
  TTree *t1 = (TTree*)f->Get("RGA_Skim4_Tree_290921_01");
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGA/Skim4/Inbending/Strangeness_2/PID/Strangeness_2_RGA_skim4_e_Kp_FD_Pass1_Inbending_Sigma_237_290921_01.root","recreate");
  // TFile fileOutput1("/media/mn688/Elements1/PhD/Analysis_Output/RGA/Skim4/Inbending/Strangeness_2/PID/Strangeness_2_RGA_skim4_e_Kp_FD_Pass1_Inbending_Sigma_237_290921_01.root","recreate");
  // TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGA/Skim4/Inbending/Strangeness_2/PID/Strangeness_2_RGA_skim4_e_Kp_FD_Pass1_Inbending_Sigma_237_130921_01.root","recreate");
  // TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_2/Strangeness_2_RGB_Inclusive_e_Kp_FD_Pass1_Inbending_Sigma_237_100921_01.root","recreate");
  ...
  auto* hinv_cascade=new TH1F("hinv_cascade","Invariant mass of p #pi^{-} #pi^{-};M(p #pi^{-} #pi^{-}) [GeV];Counts",200,0,2);
  auto* hinv_missing_cascade=new TH2D("hinv_missing_cascade","Missing #Lambda against invariant #Cascade;M(p #pi^{-} #pi^{-}) [GeV];MM(e' K^{+} K^{+}) [GeV]",200,0.5,2.0,200,0.5,2.06);
  auto* hmiss_1=new TH1F("hmiss_1","MM(e' K^{+});MM(e' K^{+}) [GeV];Counts",800,-1,3);
  auto* hmiss_1_c=new TH1F("hmiss_1_c","MM(e' K^{+});MM(e' K^{+}) [GeV];Counts",800,-1,3);
  auto* hmiss_1_sig=new TH1F("hmiss_1_sig","MM(e' K^{+});MM(e' K^{+} K^{+}) [GeV];Counts",800,-1,3);
  auto* hmiss_1_back=new TH1F("hmiss_1_back","MM(e' K^{+});MM(e' K^{+} K^{+}) [GeV];Counts",800,-1,3);
  auto* hmiss_1_phi=new TH1F("hmiss_1_phi","MM(e' K^{+});MM(e' K^{+} K^{+}) [GeV];Counts",800,-1,3);
  auto* hmass_kp=new TH1F("hmass_kp","K^{+} mass;M(K^{+});Counts",100,0.2,0.8);
  auto* hregion_kp_1=new TH1F("hregion_kp_1", "K^{+} regions; region; Counts",3,0,3);
  auto* hregion_kp_2=new TH1F("hregion_kp_2", "K^{+} regions; region; Counts",3,0,3);
  ...
  TLorentzVector lambda_1, lambda_2;
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_kp.size()==2 && v_el.size()==1){
      ...
      hregion_kp_1->Fill(v_region_kp.at(0));
      hregion_kp_2->Fill(v_region_kp.at(1));
      ...
      // if(v_region_kp.at(0) > 0.5 && v_region_kp.at(0) < 1.8 && v_region_pr.at(0) > 0.5 && v_region_pr.at(0) < 1.8){
        ...
        miss1 = (TLorentzVector)*readbeam + target - v_el.at(0) - v_kp.at(0) - v_kp.at(1);
        // miss1 = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1);

        // Filling missing mass histograms
        hmass_kp->Fill(v_Mass_kp.at(0));
        hmass_kp->Fill(v_Mass_kp.at(1));
        hmiss_1->Fill(miss1.M());
        if(fabs(v_delta_beta_kp.at(0)) < 0.02 && fabs(v_delta_beta_kp.at(1) < 0.02)) hmiss_1_c->Fill(miss1.M());
        ...
        // if(miss1.M() > 0.9 && miss1.M() < 1.3){

          // Selecting events where the pion is also detected
          if(v_pim.size()==2 && v_pr.size()==1){
            // Invariant mass of p pi^{-}, looking for lambda ground state
            lambda_1 = v_pr.at(0) + v_pim.at(0);
            lambda_2 = v_pr.at(0) + v_pim.at(1);
            // Invariant mass of p pi^{-} pi^{-}, looking for cascade ground state
            cascade = v_pr.at(0) + v_pim.at(0) + v_pim.at(1);
            // Missing mass of all detected particles, should have peak at 0
            missall = (TLorentzVector)*readbeam + target - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_pr.at(0) - v_pim.at(0) - v_pim.at(0);
            // missall = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_pr.at(0) - v_pim.at(0) - v_pim.at(0);
            // Invariant against missing mass of cascade
            hinv_missing_cascade->Fill(cascade.M(),miss1.M());
