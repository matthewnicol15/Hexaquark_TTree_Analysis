...

{
   ...
   TFile fileOutput1("Kaon_Background_Subtraction_RGA_Fall2018_Outbending_Alternative_16072022_01.root","recreate");
   ...
   for(Int_t i = 1; i < hist_S1->GetNbinsX()+1; i++){
      ...
      par5 = 0.1005*exp(-0.5*pow((momentum_S1[counter_S1]-1.628)/0.448,2))+0.012+0.04098*momentum_S1[counter_S1]; // sigma for back gaus
      ...
      func1[counter_S1]->FixParameter(3,par3); // amplitude for back gaus
      func1[counter_S1]->FixParameter(4,par4); // mean for back gaus (pion mass)
      func1[counter_S1]->FixParameter(5,par5); // sigma for back gaus
