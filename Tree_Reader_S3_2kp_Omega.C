...
void Tree_Reader_S3_2kp_Omega(){
  ...
  TFile *f = new TFile("/media/mn688/Elements1/PhD/Trees/Dibaryon/RGB/RGB_Spring2020_Inbending_at_least_1eFD_1Kp_Tree_Total_22102021.root");
  // Read TTree within root file and assign it to 't1'
  TTree *t1 = (TTree*)f->Get("RGB_Spring2020_Inbending_221021");
  ...
  // Analysis Plots
  auto* h_proton_pion_pair = new TH1F("h_proton_pion_pair",
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_el.size() != 1 || v_kp.size() < 2 || v_pr.size() < 1 || v_pim.size() < 1 || v_km.size() < 1) continue;
    {
      ...
      h_proton_pion_pair->Fill(proton_pion_1.M());
      h_inv_omega->Fill(omega_1.M());
