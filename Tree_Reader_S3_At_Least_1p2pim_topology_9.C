...
// Macro name
void Tree_Reader_S3_At_Least_1p2pim_topology_9(){
  ...
  auto* h_delta_beta_pim = new TH2F("h_delta_beta_pim","#Delta#Beta of #pi^{-}",200,0,12,100,-0.2,0.2);
  auto* h_delta_beta_pip = new TH2F("h_delta_beta_pip","#Delta#Beta of #pi^{+}",200,0,12,100,-0.2,0.2);
  auto* h_delta_beta_km = new TH2F("h_delta_beta_km","#Delta#Beta of K^{-} ",200,0,12,100,-0.2,0.2);
  auto* h_delta_beta_kp = new TH2F("h_delta_beta_kp","#Delta#Beta of K^{+} ",200,0,12,100,-0.2,0.2);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    for(Int_t j=0; j<Nparticles; j++){
      ...
      // pi^+
        ...
        // Plotting the delta beta before cutting on it
        h_delta_beta_pip->Fill(pip.Rho(),delta_beta_pip);

        // Only push back particles with good delta beta
        if(fabs(delta_beta_pip) < 0.02){
      ...
      // pi^-
        ...
        // Plotting the delta beta before cutting on it
        h_delta_beta_pim->Fill(pim.Rho(),delta_beta_pim);
      ...
      // protons
        ...
        // Plotting the delta beta before cutting on it
        h_delta_beta_proton->Fill(pr.Rho(),delta_beta_pr);

        // Only push back particles with good delta beta
        if(fabs(delta_beta_pr) < 0.02){
      ...
      // K^+
        ...
        // Plotting the delta beta before cutting on it
        h_delta_beta_kp->Fill(kp.Rho(),delta_beta_km);
      ...
      // K^-
        ...
        // Plotting the delta beta before cutting on it
        h_delta_beta_km->Fill(km.Rho(),delta_beta_km);

        // Only push back particles with good delta beta
        if(fabs(delta_beta_km) < 0.02){
