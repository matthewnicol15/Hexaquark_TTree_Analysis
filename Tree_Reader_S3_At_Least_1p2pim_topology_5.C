...
// Macro name
void Tree_Reader_S3_At_Least_1p2pim_topology_5(){
  ...
  auto* hkaonpno_topology_5 = new TH1F("hkaonpno_topology_5", "Number of K^{+};# of K^{+}; Counts",10,0,10);
  ...
  "Invariant masses;M(p #pi^{-}) [GeV];M(p #pi^{-} #pi^{-}) [GeV]; M(p #pi^{-} #pi^{-}) [GeV]",100,1,1.2,225,1,1.45,225,1,1.45);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_el.size() != 1 || v_pr.size() < 1 || v_pim.size() < 3) continue;
    {
      ...
      if(proton_pion_1.M() < 1.16){
        ...
        // Check if either pion 2 or 3 are from cascade
        if(cascade_12.M() < 1.45 || cascade_13.M() < 1.45){

          h_invariant_mass_123->Fill(proton_pion_1.M(), cascade_12.M(), cascade_13.M());
        }
      }
      ...
      if(proton_pion_2.M() < 1.16){
        ...
        // Check if pion 1 or 3 are from cascade
        if(cascade_12.M() < 1.45 || cascade_23.M() < 1.45){

          h_invariant_mass_213->Fill(proton_pion_2.M(), cascade_12.M(), cascade_23.M());
        }
      }
      ...
      if(proton_pion_3.M() < 1.16){
        ...
        // Check if pion 1 or 2 are from cascade
        if(cascade_13.M() < 1.45 || cascade_23.M() < 1.45){

          h_invariant_mass_312->Fill(proton_pion_3.M(), cascade_13.M(), cascade_23.M());
        }
      }
    }
  }
