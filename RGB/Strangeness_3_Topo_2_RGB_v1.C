...
// Macro name
void Strangeness_3_Topo_2_RGB_v1(){
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_3/PID/Strangeness_3_Topo_2_RGB_Inc_Pass1_Inbending_e_3Kp_081020_01.root","recreate");
  ...
  auto* hmisspi0=new TH1F("hmisspi0","MM^{2}(e' K^{+} K^{+} K^{+} p p #pi^{-} #pi^{-} #pi^{-} #pi^{-});MM^{2}(e' K^{+} K^{+} K^{+} p p #pi^{-} #pi^{-} #pi^{-} #pi^{-}) [GeV];Counts",200,-1,1);
  auto* hmisspi0_2=new TH1F("hmisspi0_2","MM^{2}(e' K^{+} K^{+} K^{+} p p #pi^{-} #pi^{-} #pi^{-} #pi^{-});MM^{2}(e' K^{+} K^{+} K^{+} p p #pi^{-} #pi^{-} #pi^{-} #pi^{-}) [GeV];Counts",200,-1,1);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(kaonpno==3 && v_el.size()==1){
      ...
      if(v_pr.size() == 2){
        ...
        if(v_pim.size() == 4){
          misspi0 = beam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_kp.at(2) - v_pr.at(0) - v_pr.at(1) - v_pim.at(0) - v_pim.at(1) - v_pim.at(2) - v_pim.at(3);

          hmiss_5_4->Fill(miss5.M());
          hmisspi0->Fill(misspi0.M2());
          ...
          // Applying cuts on the delta beta of K^{+}s
