...
void Strangeness_2_RGB(){
  ...
  TFile *f = new TFile("/mnt/f/PhD/Trees/Dibaryon/RGB/Strangeness_2/RGB_Spring2020_Inbending_S2_eFD_At_Least_2Kp_Tree_190821_01.root");
  // Read TTree within root file and assign it to 't1'
  TTree *t1 = (TTree*)f->Get("RGB_Spring2020_Inbending_190821");
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_2/PID/Strangeness_2_RGB_Skim4_e_Kp_FD_Pass1_Inbending_Sigma_237_290921_01.root","recreate");
  ...
  auto* hinv_cascade=new TH1F("hinv_cascade","Invariant mass of p #pi^{-} #pi^{-};M(p #pi^{-} #pi^{-}) [GeV];Counts",200,1,3);
  auto* hinv_lambdas=new TH2D("hinv_lambdas","Invariant #Lambda (1) against invariant #Lambda (2);M(p #pi^{-} (1)) [GeV];M(p #pi^{-} (1)) [GeV]",200,0.5,2.0,200,0.5,2.0);
  ...
  auto* hmiss_2=new TH1F("hmiss_2","MM^{2}(e' K^{+} p);MM^{2}(e' K^{+} K^{+} p) [GeV^{2}];Counts",800,-1,3);
  ...
  auto* h_delta_beta_kp_1=new TH2D("h_delta_beta_kp_1","K^{+} (1) Delta Beta; K^{+} P [GeV]; #Delta #Beta",100,0,12, 200,-0.2,0.2);
  auto* h_delta_beta_kp_2=new TH2D("h_delta_beta_kp_2","K^{+} (1) Delta Beta; K^{+} P [GeV]; #Delta #Beta",100,0,12, 200,-0.2,0.2);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_kp.size()==2 && v_el.size()==1 ){
      ...
      // if(v_region_kp.at(0) > 0.5 && v_region_kp.at(0) < 1.8 && v_region_pr.at(0) > 0.5 && v_region_pr.at(0) < 1.8){
        ...
        // miss2 = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_pr.at(0);
        ...
        if(fabs(v_delta_beta_kp.at(0)) > 0.02 && fabs(v_delta_beta_kp.at(1)) > 0.02) continue;
        ...
        // Cut around the missing mass of the cascade
        if(miss1.M() > 1.25 && miss1.M() < 1.45){
          ...
          if(v_pim.size()==2 && v_pr.size()==1){
            ...
            missall = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_pr.at(0) - v_pim.at(0) - v_pim.at(1);
            ...
            hinv_lambdas->Fill(lambda_1.M(), lambda_2.M());
