...
void Strangeness_3_RGB_v1(){
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_3/PIDStrangeness_3_RGB_Inc_Pass1_Inbending_e_Kp_FD_300920_02.root","recreate");
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_kp.size()==3 && v_el.size()==1){
      ...
      // Seeing the effect of 1,2,3 K+ in FD
      ...
      // Selecting events where at least 1 proton is also detected
      if(v_pr.size() > 0){
        ...
        if(v_pim.size() > 0){
          ...
          hregion_kp1->Fill(v_region_kp.at(0));
          hregion_kp2->Fill(v_region_kp.at(1));
          hregion_kp3->Fill(v_region_kp.at(2));
          ...
          for(Int_t k=0;k<protonno;k++){
            ...
            for(Int_t l=0;l<pimno;l++){
              ...
              if(lambda.M() < 1.2)hmiss_5_9->Fill(miss5.M());
            }
          }
          if(v_pim.size() > 1)hmiss_5_10->Fill(miss5.M());
        }
      }
    }
  }
