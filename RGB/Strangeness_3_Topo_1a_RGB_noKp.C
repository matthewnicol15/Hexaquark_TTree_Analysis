...
// Macro name
void Strangeness_3_Topo_1a_RGB_noKp(){
  ...
  TFile *f = new TFile("/mnt/f/PhD/Trees/Dibaryon/RGB/Strangeness_3/RGB_Spring2020_Inbending_S3_eFD_At_Least_1p2pim1km_Tree_030821_01.root");
  // Read TTree within root file and assign it to 't1'
  TTree *t1 = (TTree*)f->Get("RGB_Spring2020_Inbending_030821");
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_3/PID/Strangeness_3_Topo_1a_RGB_Spring_2020_Inc_Pass1_Inbending_eFD_1p2pim1km_chi2_open5_190821_01.root","recreate");
  ...
  auto* hinvestigation=new TH2D("hinvestigation", "Invariant mass of #Lambda (2) against #Lambda (1);M(p #pi^{-} (1)) - M(p #pi^{-} (2)) [GeV];Angle [deg]",600,-2,2,600,-180,180);
  auto* hopening_angle=new TH1F("hopening_angle","Invariant mass of p #pi^{-};M(p #pi^{-}) [GeV];Counts",600,-180,180);
  ...
  Double_t Opening_Angle; // opening angle between the two pions
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    for(Int_t j=0; j<Nparticles; j++){
      ...
      // Apply cut on the chi^2PID
      if(v_chi2PID->at(j) > 2) continue;
    ...
    if(v_km.size() < 1 || v_el.size() != 1 || v_pr.size() < 1 || v_pim.size() < 2) continue;
    {

      // Requiring exact topology 1a
      if(v_km.size() != 1 || v_el.size() != 1 || v_pr.size() != 1 || v_pim.size() != 2) continue;
      {

        // Invariant mass of possible lambdas
        lambda1 = v_pr.at(0) + v_pim.at(0);
        lambda2 = v_pr.at(0) + v_pim.at(1);


        Opening_Angle = TMath::RadToDeg()*(v_pim.at(0).Vect().Angle(v_pim.at(1).Vect()));
        // Checking if pions are the same particle being measured twice

        // Opening angle between two pions
        hopening_angle->Fill(Opening_Angle);

        // Cutting on opening angle
        if(Opening_Angle < 5) continue;

        // Opening angle against difference in invariant masses for proton and pion 1 or 2
        hinvestigation->Fill(lambda1.M() - lambda2.M(), Opening_Angle);
