...
// Macro name
void Strangeness_3_Topo_3_RGB_noKp(){
  ...
  TFile *f = new TFile("/mnt/f/PhD/Trees/Dibaryon/RGB/Strangeness_3/RGB_Spring2020_Inbending_S3_eFD_At_Least_1p2pim_Tree_030821_01.root");
  ...
  TFile fileOutput1("/mnt/f/PhD/Analysis_Output/RGB/Inclusive/Inbending/Strangeness_3/PID/Strangeness_3_Topo_3_RGB_Spring_2020_Inc_Pass1_Inbending_eFD_At_Least_1p2pim2kp_100921_01.root","recreate");
  ...
  TLorentzVector lambda1, lambda2, lambda3;
  TLorentzVector lambda;
  TLorentzVector cascade, cascade_1, cascade_2, cascade_3;
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    // if(v_el.size() != 1 || v_pr.size() < 1 || v_pim.size() < 3) continue;
    // {
      // cout<<v_el.size()<<" "<<v_pr.size()<<" "<<v_pim.size()<<endl;
      // Requiring exact topology 3
      if(v_el.size() != 1 || v_pr.size() != 1 || v_pim.size() != 3) continue;
      {
        hkaonpno->Fill(v_kp.size());

        // Requiring detection of kaons
      ...
      for(Int_t k=0;k<protonno;k++){
        ...
        for(Int_t l=0;l<pimno;l++){
          ...
          cascade_1 = v_pr.at(k) + v_pim.at(0) + v_pim.at(1);
          cascade_2 = v_pr.at(k) + v_pim.at(0) + v_pim.at(2);
          cascade_3 = v_pr.at(k) + v_pim.at(1) + v_pim.at(2);

          if(lambda.M() < 1.18){
            for(Int_t m=0;m<pimno && m!=l ;m++){
              cascade = v_pr.at(k) +v_pim.at(l) + v_pim.at(m);
      ...
      //
        ...
        lambda3 = v_pr.at(0) + v_pim.at(2);

        hinv_lambda_1->Fill(lambda1.M());
        hinv_lambda_2->Fill(lambda2.M());
        hinv_lambda_3->Fill(lambda3.M());
