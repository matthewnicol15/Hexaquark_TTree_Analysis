...
// Macro name
void Tree_Reader_S3_At_Least_1p2pim_topology_2(){
  ...
  auto* h_delta_beta_proton = new TH2F("h_delta_beta_proton","#Delta#Beta of pi",200,0,12,100,-0.2,0.2);

  // Analysis Plots
  auto* h_proton_pion_pairs = new TH2F("h_proton_pion_pairs",
  "Invariant mass of p #pi^{-};M(p #pi^{-}) [GeV];M(p #pi^{-}) [GeV]",400,1,3,400,1,3);
  ...
  for(Long64_t i=0; i<nentries;i++){
    ...
    if(v_el.size() != 1 || v_pr.size() < 1 || v_pim.size() < 2 || v_km.size() < 1) continue;
    {
      hkaonpno_topology_3->Fill(v_kp.size());

      // Looking at the delta beta of particles in each event
      ...
      // Omegas
      omega_1 = v_pr.at(0) + v_pim.at(0) + v_km.at(0); // First pion
      omega_2 = v_pr.at(0) + v_pim.at(1) + v_km.at(0); // Second pion
